...
export async function signup(params: SignUpParams) {
    //SignUpParams is in 'types' folder, actually its in an interface file, we expect here an object with variable and datatype e.g. -> uid, name, email, password
    ...
    try {
        ...
        await db.collection('users').doc(uid).set({
            name,
            email, //setting name : name, email : email as in object form
        })
        return {
            success: true,
        ...
        // : any karne se => we dont have to any typescript validations
        ...
        return {
            //general error (you could explicitly create for password and name as well)
            success: false,
...
export async function getCurrentUser(): Promise<User | null> {
    //this funct will return a promise, that will be resolved in either a user or a null
    ...
    try {
        // console.log(cookieStore + ' store tha  ' +  sessionCookie + '   this was session cookie' )// it was just for revision purpose
        const decodedClaims = await auth.verifySessionCookie(
            sessionCookie,
            true
        ) //verifying session cookie, 2nd parameter is for 'have we revoked the session or not'
        const userRecord = await db
            .collection('users')
            .doc(decodedClaims.uid)
            .get() //who is this user which is currently loggedIn and getting him as well
